...
def combine_images_to_pdf_sync(image_paths: List[str], output_pdf_path: str) -> str | None:
    if not image_paths:
        return None
    images = []
    try:
...
async def save_new_test(
    ...
    if not re.fullmatch(VALID_KEY_PATTERN, answer_key):
        await message.answer(
            ...
            "NAMUNA: <code>1a2b3c4d...</code> (Bo'sh joylar va takroriy savol raqami yoki takroriy javoblarsiz!) ‼️",
            parse_mode='HTML'
        )
        return
...
async def process_user_answers(message: Message, state: FSMContext, session_factory: async_sessionmaker[AsyncSession],
    ...
    if not re.fullmatch(VALID_KEY_PATTERN, user_answers_raw):
        await message.answer(
            "Javoblar formati noto'g'ri. Iltimos, faqat savol raqami va bitta harf usulida kiriting. "
    ...
    if len(user_answers_raw) < 2:
        await message.answer("Sizning javoblaringiz juda qisqa. Kamida bitta to'liq javobni kiriting. ‼️")
        return
